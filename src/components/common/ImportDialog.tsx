...
export function ImportDialog() {
    ...
    return (
        <div className="fadeIn fadeIn fixed top-0  z-[6001] grid h-full w-full items-center justify-center bg-gray-50/50">
            <div className="relatedGrid relative flex flex-col border border-gray-300 bg-gray-100 p-1 p-1 shadow-2xl shadow-black dark:border-gray-900 dark:bg-gray-800">
                <span className="mb-1 block w-full bg-gray-300 p-1 text-center text-base dark:bg-gray-700 dark:text-white">
                    Import data
                </span>
                <div className="mb-1 mt-1 flex">
                    <button
                        className={classes([
                            ...
                            /* selected */
                            classIfTrue(reg === "change", "block bg-gray-400 dark:bg-gray-500 dark:text-gray-900"),

                            /* not selected */
                            classIfTrue(reg !== "change", "block bg-gray-300 dark:bg-gray-700 dark:text-blue-400")
                        ])}
                    ...
                    <button
                        className={classes([
                            ...
                            /* selected */
                            classIfTrue(reg === "new", "block bg-gray-400 dark:bg-gray-500 dark:text-gray-900"),

                            /* not selected */
                            classIfTrue(reg !== "new", "block bg-gray-300 dark:bg-gray-700 dark:text-blue-400")
                        ])}
                        onClick={() => setReg("new")}
                    >
                        New Rows
                    ...
                    <button
                        className={classes([
                            ...
                            /* selected */
                            classIfTrue(reg === "deleted", "block bg-gray-400 dark:bg-gray-500 dark:text-gray-900"),

                            /* not selected */
                            classIfTrue(reg !== "deleted", "block bg-gray-300 dark:bg-gray-700 dark:text-blue-400")
                        ])}
                ...
                <div className="bottom-2 left-0 right-0 mb-1 mt-1 flex">
                    ...
                    <button
                        ...
                        onClick={async () => {
                            if (reg === "change") {
                                ...
                                importDataStore.gridInterfaceNew.getDatasource().markForDeletion(newRowsToRemove);
                                importDataStore.gridInterfaceChange.getDatasource().markForDeletion(changeRowsToRemove);
                            }

                            if (reg === "new") {
                                const selectedRows = importDataStore.gridInterfaceNew.getDatasource().getSelectedRows();
                    ...
                    <button
                        ...
                        onClick={() => {
                            ...
                            //const dataController = getDataControllerByName(importDataStore.currentcontrollerName);
                            const inputGridController = gridControllers[importDataStore.currentcontrollerName as keyof typeof gridControllers] 
                            const dsRows = inputGridController.getGridDatasource().getRows(true);
                            const primaryKeyName = importDataStore.primaryKeyName;
                            const primaryKeyMap = new Map<unknown, Record<string, unknown>>();
                            dsRows.forEach((row) => {
                                primaryKeyMap.set(row[primaryKeyName], row);
                            });

                            // loop changeset
                            const changeSet = importDataStore.dataSourceChange.getRows(true);
                            ...
                            newRows.forEach((row) => {
                                ...
                                const result = inputGridController.getGridDatasource().setData([{}], true, true);
